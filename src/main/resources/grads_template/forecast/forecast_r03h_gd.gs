...
'define maxa = max(#(valuename),lat=20.1,lat=25.65) '
'define maxb = max(#(valuename),lon=108.95,lon=117.5) '

'define maxv = max( max(#(valuename),lat=20.1,lat=25.65 ),lon=108.95,lon=117.5)'
'd maxv'
maxvalue=subwrd(result,4)

if(maxvalue>0)

'define locx=maxloc(maxa,lon=108.95,lon=117.5)'
'define locy=maxloc(maxb,lat=20.1,lat=25.65)'
'd locx'
